...
CREATE TRIGGER `tr_srvc_categories_no_self` BEFORE INSERT ON `{!!prefix!!}srvc_categories` FOR EACH ROW BEGIN IF NEW.parent_id IS NOT NULL
AND NEW.parent_id = NEW.id THEN SIGNAL SQLSTATE '45000'
SET
  MESSAGE_TEXT = 'parent_id cannot equal id';
...
CREATE TRIGGER `tr_srvc_categories_no_self_update` BEFORE
UPDATE ON `{!!prefix!!}srvc_categories` FOR EACH ROW BEGIN IF NEW.parent_id IS NOT NULL
AND NEW.parent_id = NEW.id THEN SIGNAL SQLSTATE '45000'
SET
  MESSAGE_TEXT = 'parent_id cannot equal id';
