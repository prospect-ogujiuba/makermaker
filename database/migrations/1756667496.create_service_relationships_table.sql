...
-- >>> Up >>>
CREATE TABLE
  `{!!prefix!!}srvc_service_relationships` (
    ...
    PRIMARY KEY (`id`),
    UNIQUE KEY `uk_service_relationship` (
      `service_id`,
      `related_service_id`,
      `relation_type`
    ),
...
CREATE TRIGGER `tr_service_relation_no_self_ref` BEFORE INSERT ON `{!!prefix!!}srvc_service_relationships` FOR EACH ROW BEGIN IF NEW.service_id = NEW.related_service_id THEN SIGNAL SQLSTATE '45000'
SET
...
UPDATE ON `{!!prefix!!}srvc_service_relationships` FOR EACH ROW BEGIN IF NEW.service_id = NEW.related_service_id THEN SIGNAL SQLSTATE '45000'
SET
  MESSAGE_TEXT = 'Service cannot have a relation to itself';
