...
ADD FOREIGN KEY fk_{source}_{target} ({target}_id)
    REFERENCES srvc_{target}s(id) ON DELETE {cascade};
```
...
Output SQL:
```sql
CREATE TABLE srvc_{source}_{target} (
    ...
    FOREIGN KEY fk_{source} ({source}_id) REFERENCES srvc_{source}s(id) ON DELETE CASCADE,
    FOREIGN KEY fk_{target} ({target}_id) REFERENCES srvc_{target}s(id) ON DELETE CASCADE
);
```
...
{
    return $this->belongsToMany({Target}::class, 'srvc_{source}_{target}', '{source}_id', '{target}_id');
}
```
